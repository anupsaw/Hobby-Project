                ...
                link: function (scope, element, attr, ctrl) {
                    ...
                    function getTemplateType1() {
                        ...
                        for (var j = 0; j < jlen; j++) {
                            var asCards = $('<as-cards form-type="formCtrl.formType" >');
                            if (j === 0) asCards.attr('form-title', "formCtrl.formTitle");
                            var ilen = len - (size * j) > size ? size + (size * j) : len;
                            for (var i = size * j; i < ilen; i++) {
                                var item = scope.elements[i];
                                ctrl.fields[item.name] = item;
                                var div = $('<div class="row">');
                                div.append('<as-' + item.type + '  field="formCtrl.fields.' + item.name + '" model="model" size="formCtrl.size"></as-' + item.type + '>');
                                asCards.append(div);
                            }
                    ...
                    function getTemplateType2() {
                        var asCards = $('<as-cards form-type="formCtrl.formType">')
                        asCards.attr('form-title', "formCtrl.formTitle");
                        var len = scope.elements.length;
                        for (var j = 0; j < len; j++) {
                            var childDiv;
                            var div = $('<div class="row">');
                            ...
                            for (var i = 0; (i <= 1 && (j + i) < len); i++) {
                                ...
                                childDiv.append('<as-' + item.type + '  field="formCtrl.fields.' + item.name + '" model="model" size="formCtrl.size"></as-' + item.type + '>');
                                div.append(childDiv);
                                j = j + i;
                            }
